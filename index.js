...
const { getBlobArray } = require("./blobLoader.js");
...
async function main() {
  ...
  let data;
  if (process.env.STORAGE_SAS) {
    console.log("lodin data from blob storage");
    // Alternative load through blob Storage
    data = await getBlobArray();
  } else {
    console.log("lodin data from file");
    data = hotelData["value"];
  }

  let indexDocumentsResult = await searchClient.mergeOrUploadDocuments(data);
...
async function askOpenAI(azureSearchIndexName, messages) {
  ...
  const events = await client.streamChatCompletions(chatDeploymentId, messages, {
    maxTokens: 128,
    /**
